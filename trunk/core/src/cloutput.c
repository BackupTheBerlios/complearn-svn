...
static void complearn_handle_string_list(xmlDocPtr doc, xmlNodePtr node, GArray *ss,
  ...
  while (node != NULL) {
    if (strcmp(tagname, (char *) node->name) == 0) {
      char *v = xmlNodeListGetString(doc, node->xmlChildrenNode,1);
...
static struct CLDistMatrix *fill_in_blanks(struct CLDistMatrix *clb)
{
  ...
  if (clb->cllibver == NULL)
    clb->cllibver = g_strdup(PACKAGE_VERSION);
...
GString *complearn_write_clb_dist_matrix(struct CLDistMatrix *orig)
{
  struct CLDistMatrix *inp;
  GArray *lab[2] = { NULL, NULL };
  GString *result;
  inp = complearn_clone_cldm(orig);
/*
gsl_matrix *mat, GArray *labels, GArray *cmds, const char *compressor_name)
...
GString *complearn_matrix_prettyprint_clb(LabeledMatrix *result) {
  ...
  memset(&inp, 0, sizeof(inp));
  inp.m = result;
  return clRealWriteCLBDistMatrix(&inp);
}
